...
public class TestReducer extends Reducer<IntWritable, SightingDetails, Text, Text> {
	...
	public void reduce(IntWritable key, Iterable<SightingDetails> values, Context context)  
		...
		for (SightingDetails sightDetail : values) {
			int t=0;
			int f=0;
			String format="";
			Instance instance = getInstance(sightDetail, set);
			instance.setDataset(set);
			for(Classifier classifier : classifiers){
				try {
					double[] fDistribution = classifier.distributionForInstance(instance);
					format = sightDetail.getSampleId().toString();
					//System.out.println(fDistribution[0] + "< -- >" + fDistribution[1]);
					if (fDistribution[0] > fDistribution[1]) {
						//System.out.println("Inside True and real value is " + sightDetail.getRed_Bird().get());
						t++;
					} else {
						//System.out.println("Inside False and real value is " + sightDetail.getRed_Bird().get());
						f++;
					}
				} catch (Exception e) {
					e.printStackTrace();
